...
namespace CK.DB.Actor.ActorPhoneNumber
{
    ...
    /// By default, the UK_CK_tActorPhoneNumber_PhoneNumber unique constraint enforces the fact that no PhoneNumber can be shared among actors. 
    /// </summary>
    /// <remarks>
    /// This CK.DB.Actor.ActorPhoneNumber package has been designed so that the same phone number MAY
    /// be associated to different actors: by deleting the UK_CK_tActorPhoneNumber_PhoneNumber unique constraint, it is
    ...
    public abstract partial class ActorPhoneNumberTable : SqlTable
    {
        ...
        /// <param name="isPrimary">True to set the phone numbe as the user or group's primary one.</param>
        ...
        public abstract Task<int> AddPhoneNumberAsync( ISqlCallContext ctx, int actorId, int userOrGroupId, string phoneNumber, bool isPrimary, bool? validate = null, bool avoidAmbiguousPhoneNumber = true );
